...
namespace FreeCellSolver.Test
{
    public class FoundationTests
    {
        ...
        public static IEnumerable<object[]> CanAutoPlay_testsData()
        {
            ...
            yield return new object[] { Ranks.Nil, Ranks.Ace, Ranks.Nil, Ranks.Nil, Card.Get("2C"), true };
            ...
            yield return new object[] { Ranks.Nil, Ranks.R2, Ranks.R2, Ranks.R2, Card.Get("3D"), true };  // Alow 3D -> 2C and 2S are at foundation
            // false when card can be pushed, above or equal RANK 3 and ANY oposite color cards of lower rank are NOT at foundation
            yield return new object[] { Ranks.R2, Ranks.R2, Ranks.Ace, Ranks.Nil, Card.Get("3C"), false }; // Deny 3C -> 2D not at foundation
            yield return new object[] { Ranks.Ace, Ranks.R2, Ranks.R2, Ranks.Nil, Card.Get("3C"), false }; // Deny 3C -> 2H not at foundation
            yield return new object[] { Ranks.Ace, Ranks.R2, Ranks.Ace, Ranks.Nil, Card.Get("3C"), false }; // Deny 3C -> Neither 2D or 2H are at foundation
            yield return new object[] { Ranks.Nil, Ranks.Ace, Ranks.R2, Ranks.R2, Card.Get("3D"), false }; // Deny 3D -> 2C not at foundation
            yield return new object[] { Ranks.Nil, Ranks.R2, Ranks.R2, Ranks.Ace, Card.Get("3D"), false }; // Deny 3D -> 2S not at foundation
            yield return new object[] { Ranks.Nil, Ranks.Ace, Ranks.R2, Ranks.Ace, Card.Get("3D"), false }; // Deny 3D -> Neither 2C or 2S are at foundation
        }
        ...
        {
            var f = Foundation.Create(Ranks.Nil, Ranks.Ace, Ranks.R4, Ranks.R3);
            var clone = f.Clone();
        ...
        public void ToString_returns_string_representation()
            => Assert.Equal($"HH CC DD SS{Environment.NewLine}-- AC 4D KS", Foundation.Create(Ranks.Nil, Ranks.Ace, Ranks.R4, Ranks.Rk).ToString());

        [Fact]
        public void AllCards_returns_all_cards()
        {
            var f = Foundation.Create(Ranks.R2, Ranks.Nil, Ranks.Ace, Ranks.R3);
            var allCards = f.AllCards().ToList();

            // Assert
            Assert.Equal(6, allCards.Count);
            Assert.Equal(Card.Get("AH"), allCards[0]);
            Assert.Equal(Card.Get("2H"), allCards[1]);
            Assert.Equal(Card.Get("AD"), allCards[2]);
